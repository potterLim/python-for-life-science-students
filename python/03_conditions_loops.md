...
실험 데이터를 다루다 보면 기준에 따라 값을 나누어야 할 때가 많습니다. 여러 샘플에 같은 작업을 반복해야 할 때도 자주 생깁니다.
...
`if` 뒤의 조건이 `True`이면 들여쓰기 된 코드가 실행됩니다. Python에서는 들여쓰기가 중요합니다. `if`에 속한 코드는 보통 네 칸 정도 들여써서 작성합니다.
...
`if` 조건이 참이면 첫 번째 문장이 실행됩니다. 거짓이면 `else` 아래 문장이 실행됩니다.
...
`elif`는 "앞 조건이 아니고 이번 조건이라면" 정도로 읽으면 됩니다.
...
처음에는 길어 보일 수 있습니다. 그래도 구조는 단순합니다.
