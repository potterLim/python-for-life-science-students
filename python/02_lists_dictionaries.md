...
실험 데이터는 보통 값 하나로 끝나지 않습니다. 여러 샘플, 여러 조건, 여러 측정값이 함께 등장합니다.
...
리스트는 여러 값을 순서대로 담고, 딕셔너리는 이름표가 붙은 값을 묶어둡니다.
...
이런 구조는 여러 시간의 측정값, 여러 샘플 이름, 여러 조건 이름을 담을 때 유용합니다.
...
처음에는 0부터 시작한다는 점이 낯설 수 있습니다. 하지만 실험 데이터 정리나 분석 자동화 코드를 읽다 보면 자주 보게 됩니다.
...
빈 리스트를 만든 뒤, 계산 결과를 하나씩 넣는 방식은 여러 측정값을 처리해 결과를 모을 때 자주 사용됩니다.
...
중요한 점은 평균 공식 자체가 아니라, 리스트 안의 여러 값을 한 번에 계산에 사용할 수 있다는 것입니다.
...
딕셔너리에서는 위치 번호가 아니라 이름표, 즉 key로 값을 꺼냅니다.
...
리스트는 몇 번째 값인지가 중요하고, 딕셔너리는 어떤 이름표가 붙어 있는지가 중요합니다.
...
이 구조는 처음에는 조금 복잡해 보일 수 있습니다. 하지만 의미는 단순합니다.
...
이 코드는 `samples` 리스트 안의 샘플을 하나씩 꺼내고, 각 샘플 딕셔너리에서 `name`과 `od600` 값을 출력합니다.
